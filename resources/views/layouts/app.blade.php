    ...
    <div class="container d-flex align-items-center">
      ...
      <nav id="navbar" class="navbar">
        <ul>
               @if (Route::has('login'))
                    @auth
                     <li class="dropdown active"><a href="{{ url('user/dashboard') }}"><span>{{ Auth::user()->name ?? '' }}</span> <i class="bi bi-chevron-down"></i></a>
                          <ul>
                            <li><a onclick="event.preventDefault(); document.getElementById('logout').submit();" href="{{route('logout')}}">{{ __('Log Out') }}</a></li>
                          </ul>
                        </li>
                        @else 
